let map, blastMarker, userMarker, circleEquipos, circlePersonal, currentData=null, localPlanUrl='', localPlanMime='image/png';
const DEFAULT_PLAN='plano-voladura.png';
const DEFAULT_DATA={blastName:'Voladura Atocongo',utmX:292925.533,utmY:8651156.600,zone:18,hemisphere:'S',radiusEquipos:300,radiusPersonal:500,date:'2026-06-02',time:'12:45',planUrl:''};
...
function normalizePlanUrl(url){return (url||'').trim()}
...
function renderPlan(d, localPreview){
 const box=document.getElementById('planContainer'); if(!box)return;
 box.innerHTML='';
 const raw=localPreview || normalizePlanUrl(d.planUrl) || DEFAULT_PLAN;
 const isPdf=(localPlanMime||'').includes('pdf') || raw.toLowerCase().includes('.pdf') || raw.includes('sharepoint.com') || raw.includes('1drv.ms') || raw.includes('drive.google.com');
 const notice=document.getElementById('planNotice');
 if(isPdf){
   const f=document.createElement('iframe'); f.src=raw; box.appendChild(f);
   const a=document.createElement('a'); a.className='plan-link'; a.href=raw; a.target='_blank'; a.textContent='Abrir plano en nueva pestaña'; box.appendChild(a);
 } else {
   const img=document.createElement('img'); img.src=raw; img.alt='Plano de referencia';
   img.onerror=()=>{box.innerHTML='<p class="hint">No se pudo mostrar el plano embebido. Usa el botón para abrirlo.</p>'; const a=document.createElement('a'); a.className='plan-link'; a.href=raw; a.target='_blank'; a.textContent='Abrir plano'; box.appendChild(a);}
   box.appendChild(img);
 }
 if(notice){
   if(localPreview) notice.textContent='Plano cargado localmente para previsualización.';
   else if(d.planUrl) notice.textContent='Plano cargado desde URL pública.';
   else notice.textContent='Mostrando plano referencial por defecto.';
 }
}
...
function preview(){currentData=adminData();drawData(currentData);renderPlan(currentData,localPlanUrl)}
...
async function handleFile(e){const f=e.target.files[0];if(!f)return;localPlanMime=f.type;localPlanUrl=await readAsDataUrl(f);renderPlan(adminData(),localPlanUrl)}
...
function generateLink(){currentData=adminData();drawData(currentData);renderPlan(currentData,localPlanUrl);const base=location.href.replace(/admin\\.html.*/,'index.html').replace(/\\?.*/,'').replace(/\\#.*/,'');document.getElementById('publicLink').value=base+'?v='+dateRoute()+'#data='+enc(currentData)}
